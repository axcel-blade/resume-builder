...
export default function ExperienceEditor({ data, set }) {
    ...
    const upd = (i, patch) => set({ experience: data.experience.map((e, idx) => (idx === i ? { ...e, ...patch } : e)) });
    const del = (i) => set({ experience: data.experience.filter((_, idx) => idx !== i) });
    const up = (i) => i > 0 && set({ experience: swap(data.experience, i, i - 1) });
    const dn = (i) => i < data.experience.length - 1 && set({ experience: swap(data.experience, i, i + 1) });
    ...
    return (
        ...
        <div className="space-y-4">
            ...
            <div key={e.id} className="rounded-xl border border-gray-200 p-3">
                ...
                <Text value={e.role} onChange={(v) => upd(i, { role: v })} placeholder="Role" />
                <Text value={e.company} onChange={(v) => upd(i, { company: v })} placeholder="Company" />
