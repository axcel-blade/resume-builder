...
export default function AchievementsEditor({ data, set }) {
    ...
    const upd = (i, patch) => set({ achievements: data.achievements.map((a, idx) => (idx === i ? { ...a, ...patch } : a)) });
    ...
    const dn = (i) => i < data.achievements.length - 1 && set({ achievements: swap(data.achievements, i, i + 1) });
    ...
    return (
        ...
        <div className="space-y-4">
            ...
            <div key={a.id} className="rounded-xl border border-gray-200 p-3">
                ...
                <Text value={a.title} onChange={(v) => upd(i, { title: v })} placeholder="Title (e.g., Hackathon Winner)" />
                <Text value={a.organization} onChange={(v) => upd(i, { organization: v })} placeholder="Organization / Issuer" />
                <Text value={a.year} onChange={(v) => upd(i, { year: v })} placeholder="Year" />
                </div>
                <div className="mt-2">
                <BulletsEditor items={a.bullets} onChange={(bullets) => upd(i, { bullets })} />
                </div>
            </div>
            ))}
        </div>
