...
export default function EducationEditor({ data, set }) {
    ...
    const upd = (i, patch) => set({ education: data.education.map((e, idx) => (idx === i ? { ...e, ...patch } : e)) });
    const del = (i) => set({ education: data.education.filter((_, idx) => idx !== i) });
    const up = (i) => i > 0 && set({ education: swap(data.education, i, i - 1) });
    const dn = (i) => i < data.education.length - 1 && set({ education: swap(data.education, i, i + 1) });
    ...
    return (
        ...
        <div className="space-y-4">
            ...
            <div key={e.id} className="rounded-xl border border-gray-200 p-3">
                ...
                <Text value={e.degree} onChange={(v) => upd(i, { degree: v })} placeholder="Degree / Qualification" />
                <Text value={e.school} onChange={(v) => upd(i, { school: v })} placeholder="Institution / University" />
                <Text value={e.location} onChange={(v) => upd(i, { location: v })} placeholder="Location" />
                <div className="grid grid-cols-2 gap-2">
                    <Text value={e.start} onChange={(v) => upd(i, { start: v })} placeholder="Start (YYYY-MM)" />
                    <Text value={e.end} onChange={(v) => upd(i, { end: v })} placeholder="End (YYYY-MM or Present)" />
                </div>
                </div>
                <div className="mt-2">
                <BulletsEditor items={e.bullets} onChange={(bullets) => upd(i, { bullets })} />
                </div>
            </div>
            ))}
        </div>
